...
New template publications use `tectonic-0.15.0-bundle33-template-v3`, with a separate offline package cache for diagrams, plots, tables and report styling. The original report-v1 and template-v2 profiles retain their original caches and remain executable without rewriting published versions. Template-v2 supports `graphicx`, PNG and JPEG. The private compiler validates resource bytes, checksums, limits and filenames before writing them beneath the isolated job's `assets/` directory. All job files are removed together.
...
CI stages the tested commit, then runs `scripts/deploy.sh COMMIT STAGED_DIRECTORY` on the server. Manual rollouts use the same script after CI deployment finishes or skips. A shared server lock serializes rollouts. Before replacement it saves the database dump, environment and resolved Compose configuration under `~/agent-platform-backups/`, and tags rollback images. It builds images, starts and checks all three compiler profiles, applies additive migrations, and replaces web/worker. Readiness failure restores prior images while retaining additive migrations. `scripts/deployment-pdf-smoke.ts` checks public health/login and a synthetic authenticated upload → publication → compilation → protected download, verifies email preview mode, and deletes the synthetic workflow. It sends no live email. The server records a successful commit in `~/agent-platform/deployed-commit`.

### Diagram and report styling packages (template-v3)

In addition to the v1/v2 packages, v3 warms `tikz`, `pgfplots`, `tabularx`, `multirow`, `siunitx`, `xcolor` (including table colouring), `fancyhdr`, `titlesec`, `enumitem`, `caption`, `float`, and `lastpage`. TikZ libraries: `arrows.meta`, `positioning`, `calc`, `fit`, `backgrounds`, `shapes.geometric`, `matrix`, `trees`, `patterns`, `decorations.pathreplacing`. PGFPlots libraries: `groupplots`, `statistics`, `fillbetween`; use `compat=1.18`. These support explicitly positioned diagrams, line/bar/scatter/area/statistical charts, grouped plots, richer tables, numeric formatting, and consistent headings/footers.

The package set is taken from the pinned bundle and exercised online and offline at image build time, for article/report classes at 10/11/12 points. Runtime remains offline, untrusted, with the same source/time/page/image limits. Externalization, gnuplot, Lua graph drawing and supporting data files are not supported. Use inline coordinates and keep figures inside the page. The v1/v2 caches do not gain these dependencies.

An Agent's **Required success tool** can name an attached PDF template tool or its enabled `generate_pdf` tool. Successful completion then requires a successful tool call and a current valid PDF; a failed final revision cannot complete successfully using an earlier successful call. Existing agents without a required PDF tool keep their completion rules.

The explicitly configured [Ofintech report template](../reports/ofintech/README.md) uses the official logo, fixed branding, and six content parameters for different report topics. It is not seeded or offered as a workflow creation example.
